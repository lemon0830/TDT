...
class BertForMultipleChoice(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        #                                nn.ReLU(), nn.Dropout(0.3), nn.Linear(config.hidden_size // 2, 1))
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        if labels is not None:
            ...
            binary_loss_fct = MSELoss()
            binary_labels = torch.zeros_like(reshaped_logits)
            binary_labels = binary_labels.reshape(-1, num_choices)
            binary_labels[range(binary_labels.shape[0]), labels] = 1
            binary_labels = binary_labels.view(-1)
            binary_loss = binary_loss_fct(reshaped_logits.view(-1), binary_labels)

            #normaled_logits = torch.softmax(reshaped_logits, dim=-1)
            #Entropy = - torch.sum(normaled_logits * torch.log2(normaled_logits), dim=-1)

            #loss = torch.mean(Entropy * loss + Entropy)

            loss = loss + 0.4 * binary_loss
