...
class BertForMultipleChoice(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        #self.gru = nn.LSTM(input_size = config.hidden_size,
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        #seq_lens = attention_mask.sum(1) # B, 1
        ...
        #packed_inputs = pack(hiddens, sorted_seq_lens, batch_first=True)


        #self.gru.flatten_parameters()

        #gru_hiddens, _ = self.gru(packed_inputs)

        #unpacked_inputs, _ =  unpack(gru_hiddens, batch_first=True)

        #gru_output = unpacked_inputs[desorted_indices]

        #gru_pooled = torch.mean(gru_output, dim=1)

        #pooled_output = torch.cat([outputs[1], gru_pooled], dim=-1)
        pooled_output = outputs[1]
