...
class PreTrainedTokenizer(object):
    ...
    def tokenize(self, text, **kwargs):
        ...
        def split_on_tokens(tok_list, text):
            ...
            return list(itertools.chain.from_iterable((self._tokenize(token, **kwargs) for token in tokenized_text)))
    ...
    def encode_plus(self,
        ...
        def get_input_ids(text, sep=False):
            if isinstance(text, six.string_types):
                token_sen = self.tokenize(text, **kwargs)
                #if sep:
                #    token_sen = [i if i != '#' else '[SEP]' for i in token_sen]

                #token_sen = list(filter(('[UNK]').__ne__, token_sen))
                return self.convert_tokens_to_ids(token_sen)
        ...
        second_ids = get_input_ids(text_pair, sep=True) if text_pair is not None else None
    ...
    def truncate_sequences(self, ids, pair_ids=None, num_tokens_to_remove=0, truncation_strategy='longest_first', stride=0):
        ...
        # if len(ids) // 3 >= num_tokens_to_remove:
        ...
        elif truncation_strategy == 'random_first':
            import random
            if len(ids) > num_tokens_to_remove:
                startid = random.randint(0, len(ids) - num_tokens_to_remove)
                endid = startid + num_tokens_to_remove
                overflowing_tokens = ids[:startid] + ids[endid:]
                ids = ids[startid : endid]
            else:
                num_tokens_to_remove = num_tokens_to_remove // 8
                assert len(ids) > num_tokens_to_remove * 7

                startid = random.randint(0, num_tokens_to_remove * 7)
                endid = len(ids) - num_tokens_to_remove * 7 + startid

                overflowing_tokens = ids[:startid] + ids[endid:]
                ids = ids[startid: endid]

                assert len(pair_ids) > num_tokens_to_remove
                startid = random.randint(0, num_tokens_to_remove)
                endid = len(pair_ids) - num_tokens_to_remove + startid

                overflowing_tokens += pair_ids[:startid] + pair_ids[endid:]
                pair_ids = pair_ids[startid: endid]
