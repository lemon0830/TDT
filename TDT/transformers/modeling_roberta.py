...
class RobertaForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None,
        ...
        sequence_output = outputs[0]
        mask_sequence_output = outputs[5]
        negtive_sequence_output = outputs[6]

        sequence_output = self.dropout(sequence_output)

        mask_sequence_output = self.dropout(mask_sequence_output)

        negtive_sequence_output = self.dropout(negtive_sequence_output)

        logits = self.classifier(sequence_output)

        mask_logits = self.classifier(mask_sequence_output)

        negtive_logits = self.classifier(negtive_sequence_output)
