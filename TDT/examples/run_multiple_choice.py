...
from transformers import (WEIGHTS_NAME, BertConfig,
                                  ...
                                  AlbertConfig, AlbertForMultipleChoice, AlbertTokenizer)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (global_step+1) % 1000 == 0:
                logger.info("Loss %s", str(loss.item()))
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        for key, value in results.items():
                            logger.info("  %s = %s, global_step: %s", key, str(value), str(global_step))
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    #logger.info("Average loss: %s at global step: %s", str((tr_loss - logging_loss)/args.logging_steps), str(global_step))
                    logger.info("lr: %s", str((scheduler.get_lr()[0])))
...
def predict(args, model, tokenizer, prefix="", test=False):
    pred_task_names = (args.task_name,)
    pred_outputs_dirs = (args.output_dir,)

    results = {}
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        pred_dataset = load_and_cache_examples(args, pred_task, tokenizer, evaluate=not test, test=test)
        ...
        # preds = np.argmax(preds, axis=1)
        # acc = simple_accuracy(preds, out_label_ids)
        # result = {"pred_acc": acc, "pred_loss": pred_loss}
        # results.update(result)

        preds = torch.nn.functional.softmax(preds, dim=-1)
        preds = preds.detach().cpu().numpy()

        output_pred_file = os.path.join(pred_output_dir, "is_test_" + str(test).lower() + "_pred_results.txt")

        with open(output_pred_file, "w") as writer:
            # for (i, prediction_probabilities) in enumerate(preds):
            #     output_line = "\t".join(str(class_probability) for class_probability in prediction_probabilities) + "\t"\
            #                   + out_label_ids + "\n"
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    if cached_mode == 'test':
        modelname = args.model_name_or_path.split('/')

        if 'checkpoint' in modelname[-1]:
            modelname = modelname[:-1]

        cached_features_file = os.path.join(args.data_dir, 'cached_yfyf_{}_{}_{}_{}'.format(
            cached_mode,
            list(filter(None, modelname)).pop(),
            str(args.max_seq_length),
            str(task)))
    else:
        cached_features_file = os.path.join(args.data_dir, 'cached_sentaugfalse_{}_{}_{}_{}'.format(
            cached_mode,
...
def main():
    ...
    if args.do_train:
        ...
        # print("train_dataset_exit", args.task_name)
        # exit()
    ...
    if args.do_pred and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint)
            model.to(args.device)
            predict(args, model, tokenizer, prefix=prefix, test=True)
